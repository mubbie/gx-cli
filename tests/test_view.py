...
def test_view_on_stacked_branch(stacked_repo):
...
def test_view_on_trunk(stacked_repo):
    os.chdir(stacked_repo)
...
def test_view_untracked_branch(git_repo):
    os.chdir(git_repo)
    run_git(["checkout", "-b", "experiment/thing"], cwd=git_repo)
    (git_repo / "x.py").write_text("x\n")
    run_git(["add", "x.py"], cwd=git_repo)
    run_git(["commit", "-m", "x"], cwd=git_repo)
    result = runner.invoke(app, ["view"])
    assert result.exit_code == 0
    assert "not part of a stack" in result.output


def test_view_not_git_repo(tmp_path):
    os.chdir(tmp_path)
    result = runner.invoke(app, ["view"])
    assert result.exit_code != 0
